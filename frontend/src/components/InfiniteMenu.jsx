...
class InfiniteGridMenu {
  ...
  run(time = 0) {
    ...
    this.#render();
    this._animFrame = requestAnimationFrame(t => this.run(t));
  }
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          {/* Name — far left, outside disc */}
          <h2
            className={`
              select-none absolute font-black
              text-[clamp(1.2rem,2.5vw,3rem)]
              top-1/2 -translate-y-1/2 left-[15%]
              text-[var(--text-main)] leading-tight
          ...
          {/* Speciality — far right, outside disc */}
          <p
            className={`
              select-none absolute text-right
              text-[clamp(0.8rem,1.3vw,1.3rem)] top-1/2 -translate-y-1/2 right-[22%]
              text-[var(--text-main)] leading-snug
              transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
              ${isMoving
                ? 'opacity-0 pointer-events-none duration-[100ms]'
                : 'opacity-100 pointer-events-auto duration-[500ms]'}
          ...
          {/* Arrow — center, just below disc bottom */}
          <button
            ...
            className={`
              ...
              bg-[var(--accent)] rounded-full cursor-pointer
